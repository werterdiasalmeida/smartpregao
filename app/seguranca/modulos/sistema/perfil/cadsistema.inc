...
{
  $sql= "select pflcod from perfil where pflstatus='A' and pfldsc='".$_REQUEST['pfldsc']."'";
  $usu = $db->recuperar($sql);
    ...
    if (! $_REQUEST['pflresponsabilidade']) $_REQUEST['pflresponsabilidade'] = 'N';
    if (! $_REQUEST['pflsncumulativo']) $_REQUEST['pflsncumulativo'] = 'f';
...
if (($_REQUEST['act']=='alterar') and (! is_array($msgerro)))
{
   ...
   if ($_REQUEST['pfldatafim'] and $_REQUEST['pfldatainicio'] ) {
    ...
    if (! $_REQUEST['pfldatafim'] and ! $_REQUEST['pfldatainicio'] ) 
    {
       $sql = "update perfil set pflfinalidade='".$_REQUEST['pflfinalidade']."',pfldatafim=null, pfldatainicio=null where pflcod=".$_REQUEST['pflcod'];
    }
    if ($sql <> '') {
       $saida = $db->executar($sql);
	$db->commit();
...
<tbody>
<?
$sql= "select pflcod as codigo, pfldsc as descricao, pfldatainicio, pfldatafim,sisdsc from perfil left join sistema on perfil.sisid = sistema.sisid where pflstatus = 'A'";
