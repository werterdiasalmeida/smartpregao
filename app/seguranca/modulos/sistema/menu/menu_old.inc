...
if ($_REQUEST['act'] == 'inserir')
{
   ...
   $sql = "select count(*) as total from menu where mnulink<>'' and mnulink='".$_REQUEST['mnulink']."'";
   ...
   $sql = "insert into menu (mnucod,mnucodpai,mnudsc,mnutransacao,mnulink,mnutipo,mnustile,mnuhtml, mnusnsubmenu,mnushow,mnustatus,abacod) values ("
   .$_REQUEST['mnucod'].",";
   if ($_REQUEST['mnucodpai']) $sql = $sql.$_REQUEST['mnucodpai'].","; else $sql = $sql.' null,';
   $sql = $sql .
   "'".$_REQUEST['mnudsc']."',".
...
if ($_REQUEST['act']=='alterar')
{
   ...
   $sql = "select count(*) as total from menu where mnulink<>'' and mnulink='".$_REQUEST['mnulink']."' and mnucod<>".$_REQUEST['mnucod_int'];
   $RS = $db->recuperar($sql,$res);
   if ($RS['total']>0)
   {
   ?>
   ...
   $sql = "update menu set mnucod=".$_REQUEST['mnucod'].", mnudsc='".
   $_REQUEST['mnudsc'].
   ...
   "',   mnustile='".
   $_REQUEST['mnustile'].
   "',   mnuhtml='".$_REQUEST['mnuhtml']."',mnucodpai=";
   if ($_REQUEST['mnucodpai']) $sql = $sql.$_REQUEST['mnucodpai'].","; else $sql = $sql.' null,';
	$sql = $sql." abacod = ";
if ($_REQUEST['abacod']) $sql = $sql.$_REQUEST['abacod'].","; else $sql = $sql.' null,';
   ...
   "  where mnucod=".$_REQUEST['mnucod_int'];
    $saida = $db->executar($sql);
    $db->commit();
	?><script>
              alert('Operação realizada com sucesso');
              location.href="<?=$_SESSION['sisdiretorio']?>.php?modulo=<?=$modulo.'&acao=A&mnucod_int='.$_REQUEST['mnucod']?>";
...
if ($_POST['exclui'] > 0) 
{
	$sql = "delete from perfilmenu where mnucod=".$_POST['exclui'];
    $saida = $db->executar($sql);
	
    $sql = "delete from menu where mnucod=".$_POST['exclui'];
    $saida = $db->executar($sql);
    unset($_POST['exclui']);
    $db->commit();
    $db->sucesso($modulo);    
...
?>
<?if ($_REQUEST['acao']=='I' or ($_REQUEST['acao']=='A' and $_REQUEST['mnucod_int']<>'')){?>
<?
//título da página
...
?>
<?
if($_REQUEST['mnucod_int'] and $_REQUEST["act"]=='') { 
       $sql= "select * from menu where mnucod=".$_REQUEST['mnucod_int'];
...
<? } else { 
	...
	<input type='hidden' name="mnucod_int" value=<?=$_REQUEST['mnucod_int']?>>
	...
	<input type='hidden' name='acao' value=<?=$_REQUEST['acao']?>>
      <tr>
        ...
        <td>
	        <? if (! $_REQUEST['mnucod']) $habil='S' ;else $habil= 'N';?>
		<?=campo_texto('mnucod','S',$habil,'',4,4,'####','');?>
        </td>
      </tr>
      <tr>
	...
	<?=obrigatorio();?></td>
      ...
      <tr>
        ...
        <td>
         <?
		$sql="SELECT mnucod as codigo, mnudsc as descricao FROM menu where mnutipo = " . ($mnutipo-1) . " and mnusnsubmenu='t'";
		$db->monta_combo("mnucodpai",$sql,'S',"Selecione o Menu",'','');
	 ?>
    	 <?=obrigatorio();?></td>
        </tr>
      <?}?>
...
<? } 
} elseif ($_REQUEST['acao']=='A' and !$_REQUEST['mnucod_int']){?>
<?
//título da página
...
?>
<form method="POST"  name="formulario">
<input type='hidden' name='mnucodpai'>
...
$sql= "select  case when mnusnsubmenu=true then '<img border=\"0\" src=\"../imagens/alterar.gif\" title=\"Alterar Menu\" onclick=\"altera_menu('||mnucod||')\">&nbsp;&nbsp;&nbsp;<img border=\"0\" src=\"../imagens/excluir.gif\" title=\"Excluir Menu\" onclick=\"excluir_menu('||mnucod||')\">&nbsp;&nbsp;&nbsp;<img border=\"0\" src=\"../imagens/gif_inclui.gif\" title=\"Incluir Menu em » '||mnudsc||'\" onclick=\"incluir_menu('||mnucod||','||mnutipo||')\">' else '<img border=\"0\" src=\"../imagens/alterar.gif\" title=\"Alterar Menu\" onclick=\"altera_menu('||mnucod||')\">&nbsp;&nbsp;&nbsp;<img border=\"0\" src=\"../imagens/excluir.gif\" title=\"Excluir Menu\" onclick=\"excluir_menu('||mnucod||')\">' end as acao, mnucod, case when mnutipo=2 then '&nbsp;&nbsp;<img src=\"../imagens/seta_filho.gif\" align=\"absmiddle\">'||mnudsc when  mnutipo=3 then '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=\"../imagens/seta_filho.gif\" align=\"absmiddle\">'||mnudsc when  mnutipo=4 then '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=\"../imagens/seta_filho.gif\" align=\"absmiddle\">'||mnudsc else mnudsc end as mnudsc, case when mnushow=false then '<font color=#808080>Não</font>' else '<font color=#008000>Sim</font>' end as mnushow, mnutransacao from menu where mnustatus = 'A' order by mnucod";
...
<?}?>
  ...
  function altera_menu(cod) {
     location.href = 'seguranca.php?modulo=sistema/menu/menu&acao=A&mnucod_int='+cod;
  }
  
  function incluir_menu(codpai, tipo) {
     document.formulario.mnucodpai.value = codpai;
	 document.formulario.mnutipo.value = tipo+1;
	 location.href = 'seguranca.php?modulo=sistema/menu/menu&acao=I&mnucodpai='+document.formulario.mnucodpai.value+'&mnutipo='+document.formulario.mnutipo.value;
  }
  ...
  function excluir_menu(cod) { 
    ...
    function validar_cadastro(cod) {
		...
		{		
		if (!validaBranco(document.formulario.mnucodpai, 'Menu Pai')) return;
