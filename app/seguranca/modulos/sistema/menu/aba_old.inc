...
{
	if ($acao == 'I') {
    $sql = "insert into abas (abadsc) values ('".$_POST['abadsc']."')";
    $db->executar($sql);
	$sql = "select max(abacod) as abacodmax from abas";
	...
	$sql = "update abas set abadsc='".$_POST['abadsc']."' where abacod=".$abacodmax;
    $db->executar($sql);
	}
	//inclui aba_menu
	  ...
	  if ($nlinhas >= 0)
	   {
	     for ($i=0; $i<=$nlinhas;$i++)
	     {
	       ...
	       if ($_POST[$mnucod]<>'')
	       {
	         ...
	         // procurar se já foi marcada antes. se não foi, então inserir.
	           $sql = 'select * from aba_menu where mnucod='.$mnucod.' and abacod='.$abacodmax;
	            $RS2 = $db->record_set($sql);
	            $nlinhas2 = $db->conta_linhas($RS2);
	            if ($nlinhas2 == -1) {
	              // incluir
	                $sql = 'insert into aba_menu (abacod, mnucod) values ('.$abacodmax.','.$mnucod.')';
	                 $db->executar($sql);
	            }
         ...
         // caso não tenha marcado, procurar se existe no banco. Se existir, então apagar
			...
			if ($nlinhas3 == 0)
            { // excluir
                 $sql = 'delete from aba_menu where abacod='.$_POST['abacod'].'  and mnucod='.$mnucod;
...
if ($act == 'excluir' and $acao=='A')
{
    ...
    $db->executar($sql);
	$sql = "delete from abas where abacod=". $_POST['abacod'];
    $db->executar($sql);
	$db ->commit();
    $db->sucesso('sistema/menu/aba');
...
{ 
    $sql= "select * from abas where abacod=".$_REQUEST['abacod_int'];
...
<input type='hidden' name="abacod" value="<?=$abacod?>">
	  ...
	  <tr>
		...
		if ($acao=='I') $sql = "select menu.mnucod, menu.mnudsc, menu.mnulink, menu.mnutransacao, menu.mnutipo, '' as check from menu order by menu.mnucod";
		else $sql = "select menu.mnucod, menu.mnudsc, menu.mnulink, menu.mnutransacao, menu.mnutipo, case when aba_menu.mnucod isnull then '' else 'checked' end as check from menu left join aba_menu on menu.mnucod=aba_menu.mnucod and aba_menu.abacod =".$abacod.' order by menu.mnucod';
		$RS = $db->carregar($sql);
		  $nlinhas = count($RS)-1;
		  if ($nlinhas >= 0)
		  {
		    ...
		    {	
				...
				print $space.'<input type="checkbox" name="'.$mnucod.'" value="'.$mnucod.'"'.$check.' '.$block.'>'.$mnucod.' - '.$mnutransacao.'<br>';
			}
		  }
		?></tr></table>
	</td></tr>
...
$sql= "select '<img border=\"0\" src=\"../imagens/alterar.gif\" title=\"Alterar Aba\" onclick=\"altera_aba('||abacod||')\">&nbsp;&nbsp;&nbsp;<img border=\"0\" src=\"../imagens/excluir.gif\" title=\"Excluir Aba\" onclick=\"excluir_aba('||abacod||')\">', abacod, abadsc, (select count(*) from aba_menu where aba_menu.abacod=abas.abacod) as abaqtd from abas order by abadsc";
...
<script>
  function altera_aba(cod) {
     location.href = 'seguranca.php?modulo=sistema/menu/aba&acao=A&abacod_int='+cod;
  }
