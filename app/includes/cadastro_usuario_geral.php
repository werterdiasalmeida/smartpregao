		...
		if ( $_REQUEST['formulario']) {
			...
			if ($permissao) {
				$sql = sprintf("UPDATE seguranca.usuario SET
                                ...
                                pg_escape_string((!empty($_REQUEST['carid']) ? $_REQUEST['carid'] : 'null')),
                                ...
                                pg_escape_string($_REQUEST['usumatricula']),
...
<form method="post" name="formulario">
	...
	<tr>
		<td align='right' class="SubTituloDireita">Nome de Guerra:</td>
	...
	<tr>
		<td align='right' class="subtitulodireita">Matrícula:</td>
		<td><?= campo_texto( 'usumatricula', 'N', $habilitar_edicao, '', 50, 100, '', '' ); ?>
	</tr>
	<tr>
		<td align='right' class="subtitulodireita">Patente:</td>
		<td>
			...
			?>
			<?= campo_texto( 'usufuncao', 'N', $habilitar_edicao, '', 50, 100, '','', '', '', '', 'id="usufuncao" style="display: none;"' ); ?>
...
if( ($permissao == true) && ($habilitar_edicao == 'S') ) {
	...
	<tr id="tr_senha">
		...
		<td><input id="senha" type="checkbox" name="senha" /> <label
			for="senha">Alterar a senha do usuário para a senha padrão: <b><?= SENHA_PADRAO; ?></b>.</label>
		</td>
	</tr>
	<?php
	} 
