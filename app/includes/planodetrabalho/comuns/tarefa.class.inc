...
//		protected $intId = null;
		...
		static protected $objTarefaDisparouProcesso;
		...
		protected $strElementoNome = 'Tarefa';
		
		protected $strElementoNomePlural = 'Tarefas';
		...
		public function setNovaDataInicio( $datInicio , $boolMantemDuracao = false )
		{
			if( $datInicio != null )
			{
		...
		public function setNovaDataInicioTimestamp( $intNovaData )
		{
			$this->setNovaDataInicio( date( 'd/m/Y' , $intNovaData ) );
		}
		...
		public function setNovaDataFimTimestamp( $intNovaData )
		{
			$this->setNovaDataFim( date( 'd/m/Y' , $intNovaData ) );
		}
		...
		{
			return $this->strElementoNomePlural;
		}
...
*/
		...
		protected function atualizaDatasaPartirDasPredecessoras()
		{
			if( $this->getPredecessora() !== null )
			{
				$intNovaDuracao = $this->getNovaDuracaoTimestamp();
				$this->setNovaDataInicioTimestamp(	$this->getPredecessora()->getNovaDataInicioTimestamp() + $this->TIMESTAMP_DAY );
				$this->setNovaDataFimTimestamp(		$this->getPredecessora()->getNovaDataInicioTimestamp() + $this->TIMESTAMP_DAY  +  $intNovaDuracao );
				$this->setChanged( true );
				$this->getPredecessora()->setChanged( true );
			}
		}
		...
		public function inserirTarefa()
		{
			...
			{
				$objTarefaAnterior = $objTarefaAnterior->getDescendenteDeMaiorCodigoUnico();
...
*/
		...
		public function getTarefaPeloCodigoUnico( $intTarefaCodigo , Projeto $objProjeto )
		{
			...
			if( $objSaida !== FALSE )
			{
				...
				{
					throw new Exception( 'Mais de uma ' . $this->getElementoNome(). ' por codigo unico ( ' . $intTarefaCodigo . ' ) no projeto ( ' . $objProjeto->getId() . ' ) ') ;	
				}
				return $arrResult[ 0 ];
			}
			else
		...
		public function getDescendenteDeMaiorCodigoUnico()
		{
			$arrFilhas = $this->getArraydeTarefasqueContenho();
			if( sizeof( $arrFilhas ) > 0 )
			{
				$arrFilhas = orderArrayOfObjectsByMethod( $arrFilhas , 'getCodigoUnico' );
				$objUltimaFilha = $arrFilhas[ sizeof( $arrFilhas ) - 1 ];
				return $objUltimaFilha->getDescendenteDeMaiorCodigoUnico();
			}
			else
			{
				return $this;
		...
		{
			return 'Atenção, outras ' . $this->getElementoNomePlural() . ' fazem referencia a esta. ' . 
		...
		{
			return 'Esta ' . $this->getElementoNome(). ' possui ' . $intQtdFilhas . ' filha(s). ' . 
				'Esta(s) ' . $this->getElementoNome(). '(s) serão removida(s) juntamente. ' .
				'';
		}
		...
		protected function getMensagemdeAvisoPossuiDescendentesIndiretos( $intQtdTarefasContidas )
		{
			return 'Caso confirmada, a operação estará totalizando, ' . 
				...
				'da(s) ' . $intQtdTarefasContidas . ' ' . $this->getElementoNome(). '(s) contida(s) nesta.' . 
				'';
		}
		
		protected function getMensagemdeAvisoPossuiSucessoras( $intQtdSucessoras )
		{
			return 'Esta ' . $this->getElementoNomePlural() . 'possui ' . $intQtdSucessoras . ' sucessora(s).' . 
				'Estas ' . $this->getElementoNomePlural() . ' perderão a predecessora. '.
				'';
		}
		...
		{
			return	'Não foi possível alterar as ' . $this->getElementoNomePlural() . ' sucessoras.' .
				'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . 
				' não pode ter a sua data atualizada pois a data ' .
				' da mesma é fechada.' .
				'';			
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . 
				' não pode ter a sua data atualizada pois o '.
		...
		{
			return	'Não foi possível alterar as ' . $this->getElementoNomePlural() . ' predecessoras.' .
				'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ter a sua data atualizada ' .
		...
		{
			return 	''.
				' A ' . $this->getElementoNome(). ' não pode ter como predecessora a ' . $this->getElementoNome(). ': ' . 
				$this->getCodigoUnico() . ' - ' . $this->getNome() . 
		...
		{
			return	'Não foi possível alterar alguma(s) ' . $this->getElementoNomePlural() . 
					' que contém esta tarefa.' .
					'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . 
					' não pode ter a sua data atualizada '.
					'pois a data da mesma é fechada.' .
					'';			
		}
		...
		{
			return	'Não foi possível alterar as ' . $this->getElementoNomePlural() . ' contidas nesta. ' . "\n" .
				'A ' . $this->getElementoNome(). ' ' .$this->getCodigoUnico() . ' - ' . $this->getNome() . 
				' não pode ter a sua data atualizada pois ' .
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome(). ' não pode ser predecessora de si mesma. '.
				' Não foi possível alterar as ' . $this->getElementoNomePlural() . ' contidas nesta.' .
				'';			
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ser sucessora de si mesma. '.
				'';			
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ser predecessora e contida ' . 
				'pela mesma ' . $this->getElementoNome(). '. '.
				'';			
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ser sucessora e contida ' . 
				'pela mesma ' . $this->getElementoNome(). '. '.
				'';
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ser predecessora e conter ' . 
				'uma mesma ' . $this->getElementoNome(). '. '.
				'';			
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $$this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode ser sucessora e conter ' . 
				'uma mesma ' . $this->getElementoNome(). '. '.
				'';
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome(). ' não pode conter uma ' . $this->getElementoNome(). ' que ' . 
				'já a contém. ' . 
				'';
		}
		...
		{
			return	'A ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . ' não pode conter a si mesma. ' . 
				'';
		}
		...
		{
			return ' Este código único já está em uso pela ' . $this->getElementoNome(). ' ' . $this->getCodigoUnico() . ' - ' . $this->getNome() . '.' .
				'';
		}
		...
		{
			return ' A ' . $this->getElementoNome(). ' ' . $this->getContainer()->getNome() . ' já possui um relacionamento '.
				'indireto com alguma das ' . $this->getElementoNomePlural() . ' contidas em ' . $this->getNome() . ' não podendo, ' .
				' deste modo, se tornar a ' . $this->getElementoNome(). ' que a contenha. ' .
				'';
		}
		 ...
		 */
		public function verificaPossibilidadeDeAlterar( $intNovaDataInicio, $intNovaDataFim , $boolManterDatas = false )
		{
//			dbg( $this->getCodigoEstruturado() . ' ' . 
//				date( 'd/m/Y ',  $intNovaDataInicio ) . ' ' .
//				date( 'd/m/Y ',  $intNovaDataFim ) . ' '   
//			);
			
			$intAtualDataInicio = $this->getNovaDataInicioTimestamp();
			$intAtualDataFim	= $this->getNovaDataFimTimestamp();
			
			if( $this->getPeriodoFechado() )
			{
				if	(
						( $this->getDatasAlteradas() ) 
						&&
						(
				...
				{
					// duas alterações de mundaça nesta data não podem ser	//
					// mescladas devido a esta tarefa ter o periodo fechado	//
					$strErrorMessage = $this->getMensagemdeErroTarefaNaoComporta();
					throw new Exception( $strErrorMessage . __LINE__ ); 						
				}
			}
			else
			{
				if( $boolManterDatas )
				{
					$intNovaDataInicio	= min( $intAtualDataInicio	, $intNovaDataInicio );
					$intNovaDataFim		= max( $intAtualDataFim		, $intNovaDataFim );
				}
			}
				...
				)
			{
				...
				# efetivadas com sucesso e em cascata na tarefa que contém	#
				# esta tarefa.												#
			 	$this->verificaSeHaConflitoEntreAsQueMeContem( $intNovaDataInicio , $intNovaDataFim );
				
			 	# 2. As alterações das datas de inicio e fim devem ser 		#
				# efetivadas com sucesso e em cascata nas tarefas contidas	#
				# por esta tarefas.											#
				$this->verificaSeHaConfitoEntreAsQueContenho( $intNovaDataInicio , $intNovaDataFim );
			 	
				# 3. As alterações da data de término devem ser efetivadas	#
			 	# com sucesso e em cascata nas tarefas sucessoras.			#
				$this->verificaSeHaConfitoEntreSucessoras(	$intNovaDatadeInicioDosSucessorasDeste );
				
				# 4. As alterações da data de inicio devem ser efetivadas	#
				# com sucesso e em cascata nas tarefas predecessoras.		#
				$this->verificaSeHaConfitoEntrePredecessoras(	$intNovaDatadeTerminoDosPredecessorasDeste );
			}
			else
			{
//				dbg( ':D' . $this->getCodigoEstruturado() );
			}
		}
		 ...
		 */
		protected function verificaSeHaConfitoEntreSucessoras( $intNovaDatadeInicioDosSucessoras )
		{
			$arrSucessoras = $this->getArraydeSucessoras();
			foreach( $arrSucessoras as $objTarefaSucessora )
			{
				$intSucessoraDataInicio		= $objTarefaSucessora->getNovaDataInicioTimestamp();
				$intSucessoraDataFim		= $objTarefaSucessora->getNovaDataFimTimestamp();
				$intSucessoraDuracao		= $intSucessoraDataFim - $intSucessoraDataInicio;
				
				if( $intNovaDatadeInicioDosSucessoras > $intSucessoraDataInicio )
				{
					if( $objTarefaSucessora->getDataFechada() == false )
					{
						...
						$intNovaDataFim		= $intNovaDatadeInicioDosSucessoras + $intSucessoraDuracao;
						...
						if( $objTarefaSucessora->getDatasAlteradas() )
						{
							if( $this->boolPeridoFechado )
							{
								...
								throw new Exception( $strErrorMessage . ' 2 ' . $objTarefaQueContenho->getNovaDataInicio() . ' : ' . date( 'd/m/Y' , $intContainerDataInicio ) );
							}
							else
							{
								$this->setNovaDataInicio(	date( 'd/m/Y' , min( $intNovaDataInicio		, $this->getNovaDataInicioTimestamp()	, $this->getDataInicioTimestamp() ) ) );
								$this->setNovaDataFim(		date( 'd/m/Y' , max( $intNovaDataFim		, $this->getNovaDataFimTimestamp()		, $this->getDataFimTimestamp() ) ) );
							}
						}
						else
						{
		 ...
		 */
		protected function verificaSeHaConfitoEntrePredecessoras( $intNovaDatadeTerminoDosPredecessoras )
		{
			$arrPredecessoras = $this->getArraydePredecessoras();
			foreach( $arrPredecessoras as $objTarefaPredecessora )
			{
				$intDataInicio	= $objTarefaPredecessora->getNovaDataInicioTimestamp();
				$intDataFim		= $objTarefaPredecessora->getNovaDataFimTimestamp();
				$intDuracao		= $intDataFim - $intDataInicio;
				if( $intNovaDatadeTerminoDosPredecessoras < $intDataFim )
				{
					if( $objTarefaPredecessora->getDataFechada() == false )
					{
						...
						if( $objTarefaPredecessora->getDatasAlteradas() )
						{
							if( $this->boolPeridoFechado )
							{
								// erro de alteracoes simultaneas //
								$strErrorMessage = $objTarefaQueContenho->getMensagemdeErroTarefaNaoComporta();
								throw new Exception( $strErrorMessage . ' 3 ' . $objTarefaQueContenho->getNovaDataInicio() . ' : ' . date( 'd/m/Y' , $intContainerDataInicio ) );
							}
							else
							{
								// o periodo da tarefa sera ampliado para suportar as alterações previstas
								$intNovaDataInicio	= min( $intNovaDataInicio		, $this->getNovaDataInicioTimestamp() );
								$intMovaDataFim		= max( $intNovaDataFim			, $this->getNovaDataFimTimestamp() );
		 ...
		 * @param integer $intContainerDataFim
		 */
		protected function verificaSeHaConfitoEntreAsQueContenho( $intMinhaNovaDataInicio , $intMinhaNovaDataFim )
		{
			$arrTarefasQueContenho = $this->getArraydeTarefasqueContenho();
			$arrTarefasQueContenhoAsc =  ( orderArrayOfObjectsByMethod( $arrTarefasQueContenho , 'getDataInicioTimestamp' ) );
			$arrTarefasQueContenhoDesc = array_reverse( $arrTarefasQueContenhoAsc );
			...
			foreach( $arrTarefasQueContenhoAsc as $objTarefaQueContenho )
			{
				$intFilhaDataInicio	= $objTarefaQueContenho->getNovaDataInicioTimestamp();
				$intFilhaDataFim	= $objTarefaQueContenho->getNovaDataFimTimestamp();
				$intFilhaDuracao	= $intFilhaDataFim - $intFilhaDataInicio;
				
				// se a tarefa filha comeca antes do comeco desta tarefa pai //
				if( $intFilhaDataInicio < $intMinhaNovaDataInicio )
				{
					// se existe a possibilidade de se alterar a tarefa que contenho //
					if( $objTarefaQueContenho->getDataFechada() == false )
					{
						$intFilhaNovaDataInicio		= $intMinhaNovaDataInicio;
						$intFilhaNovaDataFim		= $intMinhaNovaDataInicio + $intFilhaDuracao;
						
						if( $objTarefaQueContenho === self::$objTarefaDisparouProcesso )
						{
							$strErrorMessage = $this->getMensagemdeErroTarefaNaoComporta();
						}
						
						$objTarefaQueContenho->verificaPossibilidadeDeAlterar( $intFilhaNovaDataInicio , $intFilhaNovaDataFim );
					}
					else
					{
			...
			foreach( $arrTarefasQueContenhoDesc as $objTarefaQueContenho )
			{
				...
				// se a tarefa filha termina depois do final desta tarefa pai //
				if( $intMinhaNovaDataFim < $intFilhaDataFim )
				{
					// se existe a possibilidade de se alterar a tarefa que contenho //
					if( $objTarefaQueContenho->getDataFechada() == false )
					{
						$intFilhaNovaDataFim		= $intMinhaNovaDataFim;
						$intFilhaNovaDataInicio		= $intMinhaNovaDataFim - $intFilhaDuracao;
		 ...
		 */
		protected function verificaSeHaConflitoEntreAsQueMeContem( $intMinhaNovaDataInicio , $intMinhaNovaDataFim )
		{
			...
			if( sizeof( $arrTarefasQueMeContem ) > 0 )
			{
				...
				{
					$intDataInicioQueMeContem	= $objTarefaQueMeContem->getNovaDataInicioTimestamp();
					$intDataFimQueMeContem		= $objTarefaQueMeContem->getNovaDataFimTimestamp();
					$intDuracaoQueMeContem		= $intDataFimQueMeContem - $intDataInicioQueMeContem;
					
					// se a minha data inicio for menor do que a data inicio da tarefa que me contem // 
					if( $intMinhaNovaDataInicio < $intDataInicioQueMeContem )
					{
						// se existe a possibilidade de se alterar a tarefa que me contem //
						if	( $objTarefaQueMeContem->getDataFechada() == false )
						{
							
							$intDataInicioQueMeContem	= $intMinhaNovaDataInicio;
							$intDataFimQueMeContem		= $intMinhaNovaDataInicio + $intDuracaoQueMeContem;
							
							if( $objTarefaQueMeContem === self::$objTarefaDisparouProcesso )
							{
								$strErrorMessage = $this->getMensagemdeErroTarefaNaoComporta();
								throw new Exception( $strErrorMessage . __LINE__  );
							}
							
							$objTarefaQueMeContem->verificaPossibilidadeDeAlterar( $intDataInicioQueMeContem , $intDataFimQueMeContem );
						}
						else
						{
							// a tarefa nao pode ser alterada por se de data fechada //
					...
					// se a minha data de termino for maior do que a data termino da tarefa que me contem // 
					else if( $intMinhaNovaDataFim > $intDataFimQueMeContem )
					{
						// se existe a possibilidade de se alterar a tarefa que contenho //
						if( $objTarefaQueMeContem->getDataFechada() == false )
						{
							$intDataFimQueMeContem		= $intMinhaNovaDataFim;
							$intDataInicioQueMeContem	= $intMinhaNovaDataFim - $intDuracaoQueMeContem;
			...
			else
			{
				...
				$this->verificaSeHaConfitoEntreAsTarefasDoProjeto( $intMinhaNovaDataInicio , $intMinhaNovaDataFim );				
			}
		}
		...
		protected function verificaSeHaConfitoEntreAsTarefasDoProjeto( $intTarefaDataInicio , $intTarefaDataFim )
		{
			...
			$intProjetoDataInicio	= $objProjeto->getDataInicioTimestamp();
			$intProjetoDataFim		= $objProjeto->getDataFimTimestamp();
			$intProjetoDuracao		= $objProjeto->getDuracaoTimestamp();
			
			// se a tarefa que me contem escapa pelo limite de data de inicio //
			if	(
					( $intTarefaDataInicio < $intProjetoDataInicio )
					||
					( $intTarefaDataFim > $intProjetoDataFim )
				)
			{
				// se existe a possibilidade de se alterar a tarefa que contenho //
				if( $objProjeto->getDataFechada() == false )
				{
					$intProjetoDataInicio	= min( $objProjeto->getNovaDataInicioTimestamp() 	, $intTarefaDataInicio );
					$intProjetoDataFim		= max( $objProjeto->getNovaDataFimTimestamp() 		, $intTarefaDataFim + $intProjetoDuracao );
					
					$objProjeto->verificaPossibilidadeDeAlterar( $intProjetoDataInicio , $intProjetoDataFim );
					
					// se ja foi anteriormente alterada //
					if( $objProjeto->getDatasAlteradas() && $this->getPeriodoFechado() && ( $this->getDuracaoTimestamp() != $intProjetoDuracao ) )
					{
						// erro de alteracoes simultaneas //
				...
				{
//					$objProjeto->setNovaDataInicioTimestamp( $intTarefaDataInicio );
//					$objProjeto->setNovaDataFimTimestamp( $intTarefaDataFim );
//					// a tarefa nao pode ser alterada por se de data fechada //
...
//					throw new Exception( $strErrorMessage . __LINE__  );
		...
		public function valida()
		{
			...
			{
				throw new Exception( 'A data de termino deve ser maior que a data de inicio' . $this->getNovaDataInicio() . ' ' . $this->getNovaDataFim() . __LINE__  );
			}
//			try
			{
				# 1. O código único deve ser único no projeto 			#
...
*/				
				...
				$this->objTarefaDisparouProcesso = $this;
				$this->verificaPossibilidadeDeAlterar( $this->getNovaDataInicioTimestamp() , $this->getNovaDataFimTimestamp() , true);
				$this->setChanged( true );
				if( $this->getProjeto() )
				{
...
//				throw $ErrorObject;
		...
		public function __setAttribute( $strAttribute, $mixValue )
		{
			switch( $strAttribute )
			{
				case 'datInicio':
				{
					$this->setNovaDataInicio( $mixValue , true );
					break;	
				}
				case 'datFim':
				{
				...
				case 'intPredecessoraCodigoUnico':
				{
					...
					$this->atualizaDatasaPartirDasPredecessoras();
					break;		
				}
				default:
				{
