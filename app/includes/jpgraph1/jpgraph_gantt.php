...
//("Sanity check for automatic Gantt chart size failed. Either the width (=$width) or height (=$height) is larger than MAX_GANTTIMG_SIZE. This could potentially be caused by a wrong date in one of the activities.");
	...
	if( !$_csim ) {
		// esta linha foi comentada por GILBERTO para corrigir um erro na produção.
...
//===================================================
class TextProperty {
    var $iFFamily=FF_FONT1,$iFStyle=FS_NORMAL,$iFSize=8;
    var $iColor="black";
    var $iShow=true;
    var $iText="";
...
// PUBLIC METHODS
    function SetStyle($aStyle='')	
    {
    	if ($aStyle) $this->iFStyle = FS_BOLD;
    }
    ...
    // Specify font
    function SetFont($aFFamily,$aFStyle=FS_NORMAL,$aFSize=8) {
	$this->iFFamily = $aFFamily;
	$this->iFStyle	 = $aFStyle;
	$this->iFSize	 = $aFSize;
    }
...
//---------------
// CONSTRUCTOR	
    function GanttBar($aPos,$aLabel,$aStart,$aEnd,$aCaption="",$aStyle=0,$aHeightFactor=0.6) {
    	     //GanttBar($key, $linha[ 'ptocod' ].$linha[ 'ptodsc' ], $linha[ 'ptodata_ini' ], $linha[ 'ptodata_fim' ]);
	parent::GanttPlotObject();	
	$this->iStart = $aStart;	
	// Is the end date given as a date or as number of days added to start date?
	if( is_string($aEnd) ) {
	    // If end date has been specified without a time we will assume
