...
function SVGObject(ii,oo)
{
	...
	this.GetSVGDoc = function _GetSVGDoc()
	{
		return(this.SVGDocument);
	}
	
	this.GetScene = function _GetScene()
	{
...
function Vector(xx, yy, zz)
{
	...
	this.Add = function _VectorAdd(vv)
	{
		this.x += vv.x;
		this.y += vv.y;
		this.z += vv.z;
	...
	this.Zoom = function _VectorZoom(ff)
	{
		this.x *= ff;
  		this.y *= ff;
  		this.z *= ff;
	...
	this.Normalize = function _VectorNormalize()
	{
		var ll=0;
		ll += this.x*this.x;
		ll += this.y*this.y;
...
function Scene3D(aParentObj, azIndex, aWidth, aHeight)
{
	...
	this.ChangeViewer = function _Scene3DChangeViewer(ddTh, ddFi)
	{
	...
	this.ChangeLight = function _Scene3DChangeLight(ddTh, ddFi)
	{
		var pi_d_180 = 3.14159265/180;
	...
	this.Init = function _Scene3DInit()
	{
		if (useSVG)
		{
