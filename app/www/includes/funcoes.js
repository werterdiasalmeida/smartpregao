function CarregaDependenciaAjax()
{
    var classe            = this;
    var arvoreDependencia = new Object();
    var paramPadrao       = new Object();

    this.addDependencia = function(referencia, dependente, param, config)
    {
    	config = config || new Object;
        param  = param || {};

        if (config.posicaoHTML == '')
        	config.posicaoHTML = 'dentro';

        var evento = config.evento || 'change';
        var obj    = arvoreDependencia;

        if (typeof(obj[referencia]) != 'object'){
            obj[referencia]         = new Object();
            obj[referencia].itens   = new Array();
            obj[referencia].param   = new Array();
            obj[referencia].evento  = new Array();
            obj[referencia].config  = new Array();
        }else{
            existeRelacao = false;
            $.each(obj[referencia].itens, function (ind, val){
	                if (val == dependente && evento == obj[referencia].evento[ind]){
	                	existeRelacao = true;
	                    return;
	                }
                })
        ...
        obj[referencia].itens.push(dependente);
        obj[referencia].param.push(param);
        obj[referencia].config.push(config);

        if ($.inArray(evento, obj[referencia].evento) < 0){
            // Adiciona evento ao elemento de referencia
            addEvento(referencia, evento);
        }

        obj[referencia].evento.push(evento);
    }

    this.paramPadrao = function (param)
    {
        paramPadrao = param;
    }

    this.buscaAjax = function (idReferencia, evento)
    {
        var obj = arvoreDependencia[idReferencia];
        $.each(obj.itens, function (ind, val){
            if (obj.evento[ind] == evento){
		        if (obj.config[ind].refeVazia == 'bloqueia' && $('#'+idReferencia).val() == ''){
			        // Bloqueia ou limpa os dependentes (html)
			        bloqHTML(idReferencia, evento);
			        return;
		        }else if (obj.config[ind].tipoRefe != 'form' && $('#'+idReferencia).val() == ''){
			        // Limpa os dependentes (html)
			        clearHTML(idReferencia, evento);
			        return;
			    }
		        // Limpa os dependentes (html)
		        clearHTML(idReferencia, evento);
                var paramUrl = concatenaArray(paramPadrao, obj.param[ind]);

                if (obj.param[ind].padrao)
                	paramUrl = concatenaArray(paramUrl, jQuery.parseJSON('{"' + obj.param[ind].padrao + '":"' + $('#'+idReferencia).val() + '"}'));
                ...
                $.ajax({
                       ...
                       success  : function(data){
							$('#' + obj.config[ind].refHTML).show();

							switch(obj.config[ind].posicaoHTML){
								case 'antes':
									$('#' + obj.config[ind].refHTML).before(data);
									break;
								case 'depois':
									$('#' + obj.config[ind].refHTML).after(data);
									break;
								default:
		                            $(data).appendTo('#' + obj.config[ind].refHTML);
    ...
    function addEvento(id, evento)
    {
        $(document).ready(function (){
                $('#'+id).bind(evento, function (){classe.buscaAjax(id, evento)});
            })
    }

    function clearHTML(idReferencia, evento)
    {
    	evento = evento || ''
        var obj = arvoreDependencia[idReferencia];

        $.each(obj.itens, function (ind, val){
                if (evento == '' || obj.evento[ind] == evento){
                	if (obj.config[ind].posicaoHTML == 'dentro')
                		$('#' + obj.config[ind].refHTML).hide();

                    $('[id^='+val+']').remove();
                    if (typeof arvoreDependencia[val] == 'object'){
                        clearHTML(val);
                    }
                }
                if (evento == ''){
    ...
    function bloqHTML(idReferencia, evento)
    {
    	evento = evento || ''
        var obj = arvoreDependencia[idReferencia];
        ...
        $.each(obj.itens, function (ind, val){
                if (evento == '' || obj.evento[ind] == evento){
                	if (obj.config[ind].refeVazia == 'bloqueia'){
	                    $('[id^='+val+']').val('');
    ...
    function limpaEvento(objRefe, indice)
    {
        objRefe.itens.splice(indice, 1);
        objRefe.param.splice(indice, 1);
        objRefe.evento.splice(indice, 1);
        objRefe.config.splice(indice, 1);
    }

    function concatenaArray(array, arrayConcatenar)
    {
    	var arrayNew = new Object();
    	$.each(array, function(index, value) {
    		arrayNew[index] = value;
    	});

        if (typeof arrayConcatenar == 'object'){
            $.each(arrayConcatenar, function(index, value) {
            	arrayNew[index] = value;
                });
        }
...
function ToolTip(id){
	...
	this.config = function (param)
	{
		id 	  			= id ? id : param.id;
		idDiv 			= id + '_toolTip';
		idDivContainer 	= id + '_toolTip_container';
		url   			= param.url;
		cache 			= param.cache ? param.cache : false;

		if (typeof param.css == 'object'){
			$.each(param.css, function(index, value) {
				  css[index] = value;
				});
		}
	}

	this.open = function (cache)
	{
		createDiv();
		managerContentDiv(cache);
		managerVisibilityDiv();
	}
	...
	function createDiv()
	{
		...
		$(div).css(css)
			  .attr("id", idDiv)
			  .append( createButtonClose() )
			  .append( createContainerDiv() );
		$('#' + id).after(div);
	}
	...
	function createButtonClose()
	{
		...
		$(img).attr({src   : '../imagens/bt_menu_close.png',
					 title : 'Fechar'})
			  .css({'float'	  	: 'right',
				  	'z-index' 	: '110',
				  	'position' 	: 'absolute',
	...
	function managerVisibilityDiv()
	{
		$('#' + idDiv).slideToggle(400);
	}

	function managerContentDiv(cache)
	{
		if (!cacheCarregado){
			$.ajax({
			      url		: url,
			      ...
			      async		: false,
			      success	: function(data){
					$('#' + idDivContainer).html(data);
			      }
			   }
			);
		}
		cacheCarregado = cache ? true : false;
	}
}
				...
				if(formulario.elements[i].type == "textarea"){
					...
					if(tiposDados[cont] == "numero"){
						if(!formulario.elements[i].value){
							...
							erro[contErro] = "O campo "+formulario.elements[i].title+" é obrigatório.";
							contErro++;
						}else if(isNaN(formulario.elements[i].value)){
							existeErro = true;
						...
						if(!formulario.elements[i].value){
							...
							erro[contErro] = "O campo "+formulario.elements[i].title+" é obrigatório.";
							contErro++;
						}else if(!validar_cpf(formulario.elements[i].value)){
							existeErro = true;
					...
					if(tiposDados[cont] == "valor"){
						var valor = formulario.elements[i].value.replace(".", "");
...
{
	var j = 1;
	var valorObjeto = objeto.toString().split(".");
	var lcDecimal;
...
function addMsgErroCampo(campo, msg){
	if(campo.closest('.form-group').find('#' + campo.attr('name') + '-error:visible').length > 0){
		return;
	}

	if(campo.parent().hasClass('input-group')){
        campo.parent().after('<span id="'+campo.attr('name')+'-error" class="help-block">'+msg+'</span>');
	}else{
        campo.after('<span id="'+campo.attr('name')+'-error" class="help-block">'+msg+'</span>');
	}


	campo.closest('.form-group').addClass('has-error');

	if(campo.find('input, textarea, select').length){
    	campo.find('input, textarea, select').focus();
	}else{
		campo.focus();
	}
}
...
if(window.jQuery) {
	$(function () {
		jQuery.extend(jQuery.validator.messages, {
			...
			accept: "Por favor, selecione um arquivo v&aacute;lido.",
			extension: "Por favor, selecione um arquivo com a extens&atilde;o v&aacute;lida.",
		...
		jQuery.validator.addMethod("cpf", function (cpf) {
			if (!jQuery.trim(cpf)) {
				return true;
			}
		...
		}, "Por favor, forne&ccedil;a um CNPJ v&aacute;lido.");

        jQuery.validator.addMethod("dateGreaterThan", function(dataFimValue, element, dataInicioSelect) {
        	var dataInicioValue = $(dataInicioSelect).val();

        	if(!dataFimValue || !dataInicioValue){
				return true;
			}

            var dataFim = dataFimValue.split('/');
            dataFim = new Date(dataFim[2] + '-' + dataFim[1] + '-' + dataFim[0]);

            var dataInicio = dataInicioValue.split('/');
            dataInicio = new Date(dataInicio[2] + '-' + dataInicio[1] + '-' + dataInicio[0]);

            if (!/Invalid|NaN/.test(dataFim)) {
				return dataFim > dataInicio;
			}
		}, 'Deve ser depois da data de início.');
	});
}

function desformataValor(value){
    value = value == '' ? 0 : value;
    return parseFloat(value.toString().split('.').join('').split(',').join('.'));
}

function formataValor(value){
    return float2moeda(value);
}

function toDateObj(dataPtBr) {
	if(!dataPtBr) {
		return null;
	}

	let data = dataPtBr.split('/');
