...
function dmenu(mnu){
	...
	if(menu[14]){m14=";"+menu[14];m14ar=m14.split(";");if(m14ar[2]){m14h=m14ar[1];m14w=m14ar[2].substr(6,99)+"px";menu[14]=m14h}else{m14h=m14ar[1];m14w="98%"}
	if(_d.layers)_d.write("<layer class=\"notprint\" z-index=1 id=menuback"+mnu+" top="+(menu[1]-m14h)+" height=2 width="+m14w+" left=0 bgcolor="+hlp[1]+"></layer>");else	_d.write("<div class=\"notprint\" id=menuback"+mnu+" style=\"position:absolute;top:"+(menu[1]-m14h)+"px;width:"+m14w+";height:2px;\">&nbsp;</div>")}
...
function MScan(){sy=this.pageYOffset;sx=this.pageXOffset;if(opra){ww=window.innerWidth;wh=window.innerHeight}else if(_d.all){sy=_d.body.scrollTop;sx=_d.body.scrollLeft;if(!ie4&&!mac){if(sy==0)sy=_d.documentElement.scrollTop;if(sx==0)sx=_d.documentElement.scrollLeft;}if(_d.compatMode=="CSS1Compat"){ww=document.documentElement.clientWidth;wh=document.documentElement.clientHeight}else{ww=_d.body.clientWidth;wh=_d.body.clientHeight}}else{if(ns4){ww=this.innerWidth-16;wh=this.innerHeight-17}else{ww=_d.body.offsetWidth;if(!ww)ww=this.innerWidth-15;ww--;wh=this.innerHeight;}}if(opra&&(owh!=wh||oww!=ww)){if(MLoaded>1){location.reload()}MLoaded++;oww=ww;owh=wh;}if(sy!=osy)checs();if(!mac&&!opra&&MLoaded<2)sis();setpos();}
...
function Minit(){el=0;df=-1;if((_d.all&&_d.getElementById)&&!mac)_d.write("<"+"script>function getflta(ap){try{if(ap.filters){flta=1}}catch(e){}}<"+"/script>");for(x=0;x<parr.length;x++){if(!opra){fixb("menu"+parr[x]);popup(parr[x])}}if(ns4){_d.captureEvents(Event.MOUSEMOVE);oww=this.innerWidth-16;owh=this.innerHeight-17;window.onresize=function(){if(ww!=oww||wh!=owh)window.history.go(0)}}_d.onmousemove=getMouseXY;MScan();}for(a=1;a<menus;a++){dmenu("menu"+a)}if(mac45)Oload=window.onLoad;else Oload=window.onload;if(!ns61&&ns6||ns6hif){window.onload=mOL}else{Minit();sis();window.onload=mOL}
