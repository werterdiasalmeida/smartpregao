...
function listaAcoes(){
	...
	$acoesExistentes = $db->carregar("SELECT 
											aca.acaid,
											aca.acadsc 
										  FROM
										    painel.acao aca
										  ...
										  ORDER BY 
										    aca.acadsc");
	$count = count($acoesExistentes);

	// Monta as TR e TD com as unidades
	for ($i = 0; $i < $count; $i++){
		$codigo    = $acoesExistentes[$i]["acaid"];
		$descricao = $acoesExistentes[$i]["acadsc"];
		if (fmod($i,2) == 0){ 
			$cor = '#f4f4f4';
		} else {
			$cor='#e0e0e0';
		...
		echo "<tr bgcolor=\"".$cor."\">
				<td align=\"right\" width=\"10%\">
					<input type=\"Checkbox\" name=\"acaid\" id=\"".$codigo."\" value=\"".$codigo."\" onclick=\"retorna('".$i."');\">
...
function atribuiAcoes($usucpf, $pflcod, $acao){
	$db = new cls_banco();
	...
	if ($acao[0]){
		foreach($acao as $acaid) {
			$dadosur = $db->carregar("SELECT * FROM painel.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND acaid = '". $acaid ."'");
			if($dadosur) {
				// Se existir registro atualizar para ativo
				$db->carregar("UPDATE painel.usuarioresponsabilidade
   							   SET rpustatus = 'A', rpudata_inc= NOW()
 							   WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND acaid = '". $acaid ."'");
			} else {
				// Se não existir, inserir novo
				$db->executar("INSERT INTO painel.usuarioresponsabilidade(
            				   pflcod, usucpf, acaid, rpustatus, rpudata_inc)
    						   VALUES ('". $pflcod ."', '". $usucpf ."', '". $acaid ."', 'A', NOW());");
			}
		}
	}
	$db->commit();
...
function buscaAcoesAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			INNER JOIN 
				painel.acao aca ON ur.acaid = aca.acaid 
			WHERE 
				ur.rpustatus = 'A' AND ur.usucpf = '$usucpf' AND ur.pflcod = $pflcod AND aca.acastatus = 'A'";
...
<html>
	...
	<body leftmargin="0" topmargin="5" bottommargin="5" marginwidth="0" marginheight="0" bgcolor="#ffffff">
		...
		<form name="formassocia" action="cadastro_responsabilidade_acao.php" method="post">
...
function retorna(objeto)
{
	...
	if (document.formulario.acaid[objeto].checked == true){
		campoSelect.options[tamanho] = new Option(document.formulario.acadescricao[objeto].value, document.formulario.acaid[objeto].value, false, false);
		sortSelect(campoSelect);
	}
	else {
		for(var i=0; i<=campoSelect.length-1; i++){
			if (document.formulario.acaid[objeto].value == campoSelect.options[i].value)
