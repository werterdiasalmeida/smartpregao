...
if(!$_REQUEST['docid'] || !$_REQUEST['esdid'] || !$_REQUEST['aedid']) {
    $arrReturn = [
        'status'   => false,
        'mensagem' => 'Informações não foram passadas corretamente. Refaça o procedimento.'
    ];
    echo json_encode(utf8ArrayEncode($arrReturn));die;
}
...
if ( wf_acaoNecessitaComentario2( $aedid ) && !$cmddsc )
{
    ...
    if ( !$documento || !$acao )
    {
        $arrReturn = [
            'execute'  => true,
            'callback' => "alert",
            'params'   => 'Documento ou ação inválida.'
    ...
    $arrReturn = [
        'execute'  => true,
        'callback' => "openModalComentario",
        'params'   => array_merge($_REQUEST, ['esddscorigem'=>$acao['esddscorigem'],'aeddscrealizar'=>$acao['aeddscrealizar']])
    ];
    echo json_encode(utf8ArrayEncode($arrReturn));die;
}
// trata dado para verificacao externa
$dadosVerificacao = unserialize( stripcslashes( $verificacao ) );
if(!$dadosVerificacao) $dadosVerificacao = json_decode($verificacao,true);
...
if (wf_alterarEstado($docid, $aedid, $cmddsc, $dadosVerificacao)) {
    ...
    echo json_encode(utf8ArrayEncode($arrReturn));
    die;
} else {
